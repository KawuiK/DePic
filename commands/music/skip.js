...
module.exports = {
  ...
  run: async (client, msg, args) => {
    ...
    if (mapload) {
      if (mapload.users.includes(msg.author.id))
        return msg.reply("Ya has votado.");
      await mapload.users.push(msg.author.id);

      if (mapload.users.length > 1) {
        let skipNumber =
          1 + parseInt(msg.member.voice.channel.members.size / 2);
        msg.channel.send(
          `${msg.author.username} ha votado para saltar la canción actual. **${mapload.users.length}/${skipNumber}**`
        );
      }
      ...
      return msg.channel.send(
        `**${msg.author.username}** inició una nueva votación para saltar la canción actual. Se necesita(n) **${skipNumber}** voto(s) para saltar la canción.`
      );
    }
  },
};
